# GitHub 仓库推荐

本页面收集 Vibe Coding 相关的优质 GitHub 仓库。

## ⭐ 官方仓库

### Anthropic (Claude)
- **[anthropics/anthropic-sdk-python](https://github.com/anthropics/anthropic-sdk-python)**
  - Anthropic 官方 Python SDK
  - 用于集成 Claude API
  - ⭐ 2k+ stars
  - 最后更新：活跃维护

- **[anthropics/anthropic-sdk-typescript](https://github.com/anthropics/anthropic-sdk-typescript)**
  - Anthropic 官方 TypeScript/JavaScript SDK
  - 支持浏览器和 Node.js
  - ⭐ 1k+ stars
  - 最后更新：活跃维护

- **[anthropics/anthropic-safe-prompt-guide](https://github.com/anthropics/anthropic-safe-prompt-guide)**
  - Anthropic 的安全提示指南
  - 学习如何安全地与 AI 交互

### OpenAI
- **[openai/openai-python](https://github.com/openai/openai-python)**
  - OpenAI 官方 Python 库
  - 支持 GPT-4, GPT-3.5 等
  - ⭐ 10k+ stars

- **[openai/openai-quickstart-python](https://github.com/openai/openai-quickstart-python)**
  - OpenAI API 快速入门示例
  - 包含常见使用场景

### GitHub Copilot
- **[github/feedback](https://github.com/github/feedback)**
  - GitHub Copilot 反馈仓库
  - 提交功能建议和 bug 报告

- **[github/copilot-docs](https://github.com/github/copilot-docs)**
  - GitHub Copilot 官方文档
  - 完整的使用指南

## 🛠️ AI 编程工具

### Cursor 相关
- **[cursor-sh-docs](https://github.com/search?q=cursor+editor+docs&type=repositories)**
  - Cursor 编辑器相关文档和配置
  - 主题、插件、快捷键等

### AI 辅助开发框架
- **[Cursor AI Resources](https://github.com/topics/cursor-ai)**
  - Cursor 编辑器相关资源和配置
  - 主题、快捷键、工作流
  - 社区贡献的技巧和工具

- **[OpenHands](https://github.com/OpenHands/openhands)**
  - AI 驱动的软件开发 agent（原 Devin 开源）
  - 可以自动完成复杂的编程任务
  - ⭐ 33k+ stars | 活跃维护
  - 支持多语言和框架

- **[Aider](https://github.com/paul-gauthier/aider)**
  - Git 集成的 AI 结对编程工具
  - 命令行界面的 AI 编程助手
  - ⭐ 12k+ stars | 活跃维护
  - 支持 Claude, GPT-4, 本地模型

- **[Continue](https://github.com/continuedev/continue)**
  - 开源的 AI 编程助手
  - 支持 VSCode, JetBrains, Chrome
  - ⭐ 20k+ stars | 活跃维护
  - 可连接多种 LLM 后端

- **[Sweep](https://github.com/SweepAI/sweep)**
  - AI 驱动的 GitHub Issue 自动处理
  - 自动修复 bugs 和实现功能
  - ⭐ 7.2k+ stars | 活跃维护
  - 类似 AI 版的 DevOps 工具

- **[Cursor Rules](https://github.com/PatrickJS/awesome-cursor-rules)** ⚠️ 待确认
  - Cursor 编辑器的自定义规则集合
  - 提升 AI 代码生成的质量

## 📚 学习资源

### Prompt Engineering
- **[dair-ai/Prompt-Engineering-Guide](https://github.com/dair-ai/Prompt-Engineering-Guide)**
  - 全面的提示工程指南
  - ⭐ 50k+ stars
  - 包含原则、技巧、案例

- **[f/awesome-chatgpt-prompts](https://github.com/f/awesome-chatgpt-prompts)**
  - 精选 ChatGPT 提示词
  - ⭐ 100k+ stars
  - 包含编程相关的 prompts

- **[brexhq/prompt-engineering](https://github.com/brexhq/prompt-engineering)**
  - Brex 公司的提示工程最佳实践
  - 实战经验分享

### AI Coding 教程
- **[microsoft/generative-ai-for-beginners](https://github.com/microsoft/generative-ai-for-beginners)**
  - 微软的生成式 AI 入门课程
  - ⭐ 50k+ stars
  - 包含 AI 编程章节

- **[ai-for-beginners/ai-for-beginners](https://github.com/microsoft/ai-for-beginners)**
  - AI 基础知识课程
  - 了解 AI 如何工作

## 🎯 实战项目示例

### AI 构建的应用
- **[topic: built-with-copilot](https://github.com/topics/built-with-copilot)**
  - 使用 Copilot 构建的项目集合
  - 学习实际应用案例

- **[topic: ai-generated](https://github.com/topics/ai-generated)**
  - AI 生成代码的项目
  - 各种语言和框架

### 框架和模板
- **[humanstack/vibe-coding-template](https://github.com/humanstack/vibe-coding-template)**
  - 完整的 Vibe Coding 全栈模板
  - 包含代码上下文，帮助 AI 编码代理更快构建应用
  - 减少 token 使用，提高开发效率
  - ⭐ 新兴项目 | 2025 最新

- **[Nanobot](https://github.com/nanobot-ai/nanobot)** ⚠️ 待确认
  - AI 驱动的项目脚手架
  - 快速启动 vibe coding 项目

- **[ai-fullstack-developer-template](https://github.com/search?q=ai+template+fullstack)**
  - 搜索 AI 辅助的全栈模板
  - 预配置好的项目结构

## 🚀 2024-2025 新兴项目 / Emerging Projects (2024-2025)

### 最新热门仓库 / Trending New Repositories
- **[bolt](https://github.com/stackblitz/bolt)**
  - StackBlatz 的 AI 全栈开发工具
  - ⭐ 5.8k+ stars | 快速增长
  - 类似 bolt.new 的开源实现

- **[Cline](https://github.com/all-hands-ai/Cline)**
  - Cursor 风格的 AI 编程助手
  - ⭐ 3.2k+ stars | 新兴项目
  - 自动编码和调试

- **[OpenDevin](https://github.com/OpenDevin/OpenDevin)**
  - 开源 AI 软件工程师
  - ⭐ 28k+ stars | 爆发式增长
  - 类似 Devin 的开源实现

- **[Pythagora](https://github.com/Pythagora-io/pythagora)**
  - AI 驱动的全栈开发工具
  - ⭐ 4.5k+ stars | 新兴
  - 自动生成和测试代码

### 专业代码模型 / Specialized Code Models
- **[deepseek-ai/deepseek-coder](https://github.com/deepseek-ai/deepseek-coder)**
  - DeepSeek 开源代码模型
  - ⭐ 8.5k+ stars | 活跃维护
  - 支持 80+ 编程语言

- **[bigcode/santacoder](https://github.com/bigcode/santacoder)**
  - BigCode 项目的小型代码模型
  - ⭐ 2.1k+ stars
  - 适合本地部署

- **[mistralai/codestral](https://github.com/mistralai/codestral)**
  - Mistral AI 的代码模型
  - ⭐ 1.5k+ stars | 新发布
  - 高性能代码生成

### 实验性工具 / Experimental Tools
- **[Aider's Code Editing](https://github.com/paul-gauthier/aider)**
  - 实验性的 AI 代码编辑功能
  - 持续创新中
  - 探索 AI 编程边界

- **[smol-ai/developer](https://github.com/smol-ai/developer)**
  - 小型 AI 开发助手
  - ⭐ 10k+ stars | 轻量级
  - 本地优先的设计

### Web 应用生成器 / Web App Generators
- **[v0](https://github.com/vercel/v0)**
  - Vercel 的 AI UI 生成器
  - 官方支持
  - 快速原型开发

- **[lovable](https://github.com/lovablehq/lovable)**
  - AI 应用构建平台
  - 快速增长
  - 全栈应用生成

## 🔬 研究和实验

### LLM for Code
- **[bigcode/bigcodebench](https://github.com/bigcode/bigcodebench)**
  - 大型代码模型基准测试
  - 了解 AI 编码能力评估

- **[openai/evals](https://github.com/openai/evals)**
  - OpenAI 的评估框架
  - 如何测试 AI 编程能力

- **[CodeLlama](https://github.com/facebookresearch/codellama)**
  - Meta 的开源代码 LLM
  - 可以本地部署

### Agent Frameworks
- **[langchain-ai/langchain](https://github.com/langchain-ai/langchain)**
  - 流行的 LLM 应用框架
  - ⭐ 95k+ stars | 活跃维护
  - 构建 AI agent 和应用

- **[microsoft/autogen](https://github.com/microsoft/autogen)**
  - 微软的多 Agent 对话框架
  - ⭐ 30k+ stars | 活跃维护
  - 支持多 Agent 协作编程

- **[langchain-ai/langgraph](https://github.com/langchain-ai/langgraph)**
  - 状态机式 Agent 框架
  - ⭐ 9.2k+ stars | 活跃维护
  - 适合构建复杂的 AI 工作流

- **[crewAIInc/crewAI](https://github.com/crewAIInc/crewAI)**
  - 角色扮演 AI Agent 框架
  - ⭐ 12k+ stars | 活跃维护
  - 快速构建专业化的 Agent 团队

- **[autogpt/autogpt](https://github.com/autogpt/autogpt)**
  - 自主 AI agent
  - ⭐ 162k+ stars | 活跃维护
  - 自动完成复杂任务

- **[RAGFlow](https://github.com/infiniflow/ragflow)**
  - RAG（检索增强生成）框架
  - ⭐ 15k+ stars | 活跃维护
  - 用于知识库驱动的 AI 编程

## 📖 文档和指南

### Awesome Lists
- **[ai-coding](https://github.com/search?q=awesome+ai+coding)**
  - 搜索 AI 编程相关的 awesome list
  - 精选资源集合

- **[awesome-llm-apps](https://github.com/shubhende/awesome-llm-apps)**
  - LLM 应用集合
  - 包含编程工具

## 🌐 社区和讨论

### 讨论和交流
- **[Awesome AI Discussions](https://github.com/search?q=ai+coding+discussion)**
  - GitHub Discussions 中关于 AI 编程的话题

### 贡献指南
如果你想推荐更多仓库，请提交 PR，包含：
- 仓库链接
- 简要描述（中文）
- Star 数量
- 推荐理由
- 最后活跃时间

---

**注意**: 本列表会持续更新。添加仓库时请确保：
1. 仓库活跃且维护良好
2. 与 Vibe Coding 高度相关
3. 有清晰的文档和示例
4. 社区认可度高（stars、forks、issues 讨论）

**更新日期**: 2025-01-05
